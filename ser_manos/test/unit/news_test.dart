...
void main() {
  ...
  setUpAll(() async {
    ...
    for (var news in news_mocked_list) {
  ...
  test("get all news.", () async {
    ...
    expect(news, news_mocked_list);
    }
  );

  test("get news by id", () async {
    for (int i = 0; i < news_mocked_list.length; i++) {
      final News retrievedNew = await newsRepositoryImpl.getNewsById(news_mocked_list[i].id.toString());
      expect(retrievedNew, news_mocked_list[i]);
    }
    }
  );
