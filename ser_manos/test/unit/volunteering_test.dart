...
void main() {
  ...
  setUpAll(() async {
    ...
    volunteeringRepositoryImpl =
        VolunteeringRepositoryImpl(firestore: fakeFirestore);
    for (var volunteering in mockedVolunteeringList) {
      await fakeFirestore
          .collection('volunteering')
          .doc(volunteering.id)
          .set(volunteering.toJson());
    }
  });

  test("get all volunteerings.", () async {
    final volunteerings = await volunteeringRepositoryImpl.getVolunteerings();
    expect(volunteerings, mockedVolunteeringList);
  });
}
